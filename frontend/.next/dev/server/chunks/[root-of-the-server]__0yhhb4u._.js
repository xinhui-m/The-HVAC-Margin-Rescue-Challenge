...
"[project]/lib/agent.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
function calculateRiskScore(project) {
    ...
    // Determine severity
    const severity = riskScore >= 60 ? "critical" : "warning";
    return {
        ...project,
    ...
    if (laborBudget > 0 && laborActual > laborBudget) {
        ...
        if (overrunPct > 0.10) {
            causes.push({
                ...
                details: multiplier > 4 ? `Crew ramped to 12-18 workers/day through peak phase; estimate assumed 5-8.` : multiplier > 2 ? `${Math.round((multiplier - 1) * 10)} additional site surveys performed outside original scope; crews worked weekends for ${Math.round(multiplier * 3)} weeks.` : `6-week schedule slip required overtime to meet deadline; premium rates not recovered.`
            });
        }
    }
    ...
    if (materialBudget > 0 && materialActual > materialBudget) {
        ...
        if (overrunPct > 0.10) {
            causes.push({
                ...
                details: multiplier > 2 ? `Late-stage delivery clustering suggests expediting and substitutions.` : `Steel price escalation of 18% not passed through; escalation clause exists but was not activated.`
            });
        }
    }
...
// Recommendations
function generateRecommendations(project, rootCauses) {
...
function generateAgentReport(topN = 10) {
    ...
    for (const project of atRisk){
        ...
        const billingCompletion = project.actual_cost > 0 ? project.billed_amount / project.actual_cost : 1;
...
var __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$agent$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/lib/agent.ts [app-route] (ecmascript)");
;
;
async function GET() {
    const report = (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$agent$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["generateAgentReport"])();
    return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$2$2e$0_react$2d$dom$40$19$2e$2$2e$4_react$40$19$2e$2$2e$4_$5f$react$40$19$2e$2$2e$4$2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json(report);
}
}),
];

//# sourceMappingURL=%5Broot-of-the-server%5D__0yhhb4u._.js.map